...
class PID (object):
   ...
   def update (self, value, targetValue):
      ...
      thrust = max(min(output, self.m_maxOutput), self.m_minOutput)
      rospy.loginfo("the thrust is %f",thrust)
      '''
      if original_thrust >= -2 and original_thrust < 2:
         thrust = 0
      elif original_thrust >= -6 and original_thrust < -2:
         thrust = -4
      elif original_thrust >= -10 and original_thrust < -6:
         thrust = -8
      elif original_thrust >= 2 and original_thrust < 6:
         thrust = 4
      elif original_thrust >= 6 and original_thrust <= 10 :
         thrust = 8
      rospy.loginfo("thrust is :%f", output)
      '''
